...
| [#818](https://github.com/pieces-app/support/issues/818) | Pro user's LLM cannot be upgraded to the latest version, even if they are chosen to | 5.35 |

## Common Issues
### 1. LTM and Workstream Issues on macOS
Users on macOS are experiencing issues with LTM and Workstream functionality, including queries not returning results, slow performance, and failure to record new activities.  The "Clear LTM Data" button is also reported as non-functional.  One user encountered a "Pieces Pro required" error despite having a Pro subscription.  Issues appear across different Pieces OS versions.
...
Users report being unable to access Pieces Pro features after upgrading. Issues include LLM not upgrading to the latest version, browser extensions failing to connect to Pieces OS, and "Pro required" errors for Pro users.  Troubleshooting includes verifying Pro status, checking configuration files, restarting, and VPN/proxy adjustments. Connectivity issues may be related to network restrictions or VPN configurations.
...
### 4. Pieces Desktop and OS Intermittently Lose Connection
Users report intermittent connection issues between Pieces Desktop and OS. Symptoms include MCP in Cursor dropping every 5-10 minutes on macOS and connection leaks on Windows when Desktop is running.  A temporary workaround involves toggling MCP or closing Pieces Desktop.  Additionally, Auto-Generated Context settings revert to "Medium" despite user changes across multiple platforms and builds.
...
## Recommendations
- Prioritize resolving LTM and Workstream issues on macOS, focusing on query functionality, performance, and the 'Clear LTM Data' button.
- Investigate and address Pieces MCP connection instability, particularly hangs and leaks associated with the Pieces desktop app.
- Ensure Pieces Pro features are accessible after upgrade, addressing LLM version discrepancies, browser extension connectivity, and 'Pro required' errors.
- Stabilize Pieces Desktop and OS connection, focusing on intermittent drops in Cursor on macOS and connection leaks on Windows.
- Address Pieces Desktop app bugs, including file drag-and-drop, sharing snippets, and unresponsive hotkeys on macOS.
